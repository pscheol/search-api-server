...
import com.search.common.logger.DefaultLogger
...
class NaverErrorDecoder(
    ...
    override fun decode(methodKey: String, response: Response): Exception {
        try {
            ...
            throw SearchAPIException(errorResponse.errorMessage, ErrorType.EXTERNAL_API_ERROR, HttpStatus.valueOf(response.status()))
        } catch (e: IOException) {
            log.error(e) { "Naver Error message parsing failed. [code=${response.status()}, request=${response.request()}, methodKey=$methodKey]" }
            throw SearchAPIException("Naver Error message parsing failed.", ErrorType.EXTERNAL_API_ERROR, HttpStatus.valueOf(response.status()))
        }
    }
}