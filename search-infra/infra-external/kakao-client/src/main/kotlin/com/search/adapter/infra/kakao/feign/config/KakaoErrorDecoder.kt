...
class KakaoErrorDecoder(
    ...
    override fun decode(methodKey: String, response: Response): Exception {
        try {
            ...
            throw SearchAPIException(errorResponse.message, ErrorType.EXTERNAL_API_ERROR, HttpStatus.valueOf(response.status()))
        } catch (e: IOException) {
            log.error(e) { "Kakao Error message parsing failed. [code=${response.status()}, request=${response.request()}, methodKey=$methodKey]" }
            throw SearchAPIException("Kakao Error message parsing failed.", ErrorType.EXTERNAL_API_ERROR, HttpStatus.valueOf(response.status()))
        }
    }
}